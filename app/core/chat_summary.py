...
def _setup_chat_summary_logger() -> None:
    ...
    try:
        ...
        handler = logging.handlers.RotatingFileHandler(
            file_path, maxBytes=settings.LOG_MAX_BYTES,
...
def build_summary_box(*, request_id, customer_id, super_admin_id, user_email, user_role,
                      conversation_id, question, exact_cache_status, semantic_cache_status,
                      vector_chunks=None, keyword_chunks=None, merged_chunks=None,
                      embedding_model, llm_model, prompt_tokens=0, completion_tokens=0,
                      total_tokens=0, embedding_time_sec=None, llm_time_sec=None,
                      total_time_sec, exact_cache_written, semantic_cache_written,
                      chat_history_saved, http_status) -> str:
    border = "═" * _BOX_WIDTH
    rows = [
        border, "AI CHAT REQUEST SUMMARY", border,
        ...
        # Question text is intentionally excluded from logs. The database
        # activity_logs row is the controlled encrypted record of the question.
        _row("Question", f"{len(question or '')} characters"),
    ...
    rows.extend([
        _row("Embedding Model", embedding_model), _row("LLM Model", llm_model),
        _row("Prompt Tokens", prompt_tokens), _row("Completion", completion_tokens),
    ...
    rows.extend([
        _row("Total API Time", f"{total_time_sec:.2f} sec"),
        _row("Exact Cache", "Written" if exact_cache_written else "Not written"),
        _row("Semantic Cache", "Written" if semantic_cache_written else "Not written"),
        _row("Chat History", "Saved" if chat_history_saved else "Not saved"),
        _row("Status", f"HTTP {http_status}"), border,
    ])
    return "\n".join(rows)
